...
dbGetQuery(db, 'SELECT GRINST AS State, COUNT(*) AS Total FROM Person WHERE GRINST = " Not in universe"')

# The number in the job type the highest earner has
dbGetQuery(db, 'SELECT AMJIND AS Job, COUNT(*) AS Total FROM Job WHERE AMJIND = " Other professional services"')

# The number in the major industry the highest earner is in
dbGetQuery(db, 'SELECT AMJOCC AS Industry, COUNT(*) AS Total FROM Job WHERE AMJOCC = " Professional specialty"')
