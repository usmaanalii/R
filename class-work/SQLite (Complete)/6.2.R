...
# Average Wage
dbGetQuery(db, 'SELECT AMJIND AS Industry, AVG(AHRSPAY) AS "Average hourly wage" FROM Hispanics 
           GROUP BY AMJIND ORDER BY AVG(AHRSPAY) DESC')

# Average Weeks
dbGetQuery(db, 'SELECT AMJIND AS Industry, AVG(WKSWORK) AS "Average weeks worked" FROM Hispanics 
